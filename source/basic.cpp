...
card cardcons(cardname cardName) {
    card c;
    c.name = cardName;
    c.cost = originalcost_c(cardName);
    c.health = originalhealth_c(cardName);
    return c;
}
...
oxy oxycons(int x, int y, const std::vector<cardname> &etcChoose) {
    //oxy:x-y操作
    oxy c;
    c.x = x;
    c.y = y;
    c.etcChoose = etcChoose;
    return c;
}

ope::ope(int cost, cardname name, const std::vector<cardname> &choose):
    cost(cost), name(name), target(invalid_m), choose(choose) {}
...
ope exact(state a, oxy b) {
    ...
    if (!b.etcChoose.empty()) {
        return ope(a.hands[b.x].cost, a.hands[b.x].name, b.etcChoose);
    }
