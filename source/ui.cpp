...
void drawatt() {
    textr(_all, txc, "你已经阅读过https://github.com/Steel-Shadow/HS-Alexstrasza-Rogue-Cal，并且该 exe 是直接获取的最新版本");
...
void getdoms() {
    ...
    //1
    rep(i, 0, 9)
        adddd(200 + i * 60, 560, 260 + i * 60, 600, vnx, clr_s(), 0, handsdomid[i] = 2 + i, 1200000 + 1000 * i, 10);
    //2 ... 11
    rep(i, 0, 9)
        adddd(200 + i * 60, 600, 260 + i * 60, 640, vx0, "", 0, chdomid[i] = 201 + i, 1100000, 5);
    //201 ... 210
    rep(i, 0, 6)
        adddd(200 + i * 60, 520, 260 + i * 60, 560, vn, clr_s(), 0, fieldsdomid[i] = 101 + i, 1300000 + 1000 * i, -1);
    //101 ... 107
    rep(i, 0, 6)
        adddd(200 + i * 60, 480, 260 + i * 60, 520, vx0, "", 0, fmhdomid[i] = 301 + i, 1400000, 5);
    //301 ... 307
    rep(i, 0, 6)
        adddd(200 + i * 60, 440, 260 + i * 60, 480, vx0, "", 0, fmchdomid[i] = 401 + i, 1500000, 5);
    //401 ... 407
    rep(i, 0, 9)
        adddd(200 + i * 60, 360, 260 + i * 60, 400, vn, clr_s(), 0, deckmdomid[i] = 1001 + i, 9300000 + 1000 * i, -1);
    //1001 ... 1010
    adddd(40, 420, 80, 460, c0, "", 0, 12, 8, -1);
    //12
    rep(i, 0, 9)
        adddd((i + 2) % 3 * 40, 420 + (i + 2) / 3 * 40, 40 + (i + 2) % 3 * 40, 460 + (i + 2) / 3 * 40, cx, "", i, 13 + i, 48 + i, -1);
    //13 ... 22
    rep(i, 0, countn - 1)
        adddd(i % 4 * 40, 60 + i / 4 * 40 + (i >= splitn) * 40, 40 + i % 4 * 40, 100 + i / 4 * 40 + (i >= splitn) * 40,
    ...
    //45
    rep(i, 0, 3)
        adddd(320 + 120 * i, 280, 440 + 120 * i, 320, vx, a1[i], 0, aurasdomid[i] = 46 + i, -1, alim[i]);
    //46 ... 49
...
cardname str2cn(const std::string& s) {
    ...
    if (s == fishin_s()) return fishin;
    return invalid;
}

minionname str2mn(const std::string &s) {
    if (s == shxk_s()) {
